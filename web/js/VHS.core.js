...
function addVideoPreview(nodeType, isInput = true) {
    chainCallback(nodeType.prototype, "onNodeCreated", function () {
        ...
        element.addEventListener('contextmenu', (e) => {
            if (e.target.tagName === 'VIDEO') return; // Allows right-clicking to save video
            e.preventDefault()
            return app.canvas._mousedown_callback(e)
        }, true);
        element.addEventListener('pointerdown', (e) => {
            if (e.target.tagName === 'VIDEO') return; // Allows clicking play/pause/timeline
            e.preventDefault()
            return app.canvas._mousedown_callback(e)
        }, true);
        ...
        element.addEventListener('pointermove', (e) => {
            if (e.target.tagName === 'VIDEO') return; // Allows scrubbing the timeline smoothly
            e.preventDefault()
            return app.canvas._mousemove_callback(e)
        }, true);
        element.addEventListener('pointerup', (e) => {
            if (e.target.tagName === 'VIDEO') return; // Allows releasing clicks on controls
            e.preventDefault()
