...
async def view_video(request):
    ...
    if query.get("format", "video") == "folder":
        ...
        concat_file = os.path.join(
            folder_paths.get_temp_directory(), "image_sequence_preview.txt"
        )
    ...
    try:
        ...
        try:
            ...
            while len(bytes_read := await proc.stdout.read(2**20)) != 0:
                try:
                    ...
                    print(
                        f"[ERROR INFO]: Connection errored, but I will proceed: {e}"
                    )
                    break
        ...
        print(f"Caught a broken Pipe: {e}")
    return resp
...
async def view_audio(request):
    ...
    if "start_time" in query:
        start_time = float(query["start_time"])
    args = (
        [ffmpeg_path, "-v", "error", "-vn"] + in_args + ["-ss", str(start_time)]
    )
        ...
        try:
            with av.open(filepath) as cont:
                ...
                source["frames"] = stream.metadata.get(
                    "NUMBER_OF_FRAMES",
                    round(source["duration"] * source["fps"]),
                )
    ...
    for item in os.scandir(path):
        try:
            ...
            if (
                ...
                or item.name.split(".")[-1].lower() in valid_extensions
            ):
                valid_items.append(item.name)
        except OSError:
